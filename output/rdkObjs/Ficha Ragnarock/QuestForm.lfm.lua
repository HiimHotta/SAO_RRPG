...
local function constructNew_frmQuestForm()
    ...
    obj._e_event2 = obj.button2:addEventListener("onClick",
        function (_)
            dialogs.confirmOkCancel("Tem certeza que quer apagar essa quest?",
            						function (confirmado)
            							if confirmado then
            								ndb.deleteNode(sheet);
            							end;
...
function newfrmQuestForm()
    ...
    __o_Utils.tryFinally(
      function()
        retObj = constructNew_frmQuestForm();
      end,
      function()
...
Firecast.registrarForm(_frmQuestForm);
